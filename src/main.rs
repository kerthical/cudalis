...
async fn main() {
    ...
    client.execute_commands(
        ...
        vec![
            ...
            "apt update -y",
            &format!(
                "apt install -y python{}-full python{}-dev python{}-distutils",
                version.get_python_semantic_version(),
                version.get_python_semantic_version(),
                version.get_python_semantic_version(),
            ),
            ...
            &format!(
                "python -m pip install torch=={} -f https://download.pytorch.org/whl/{}",
                version.torch,
                version.accelerator,
            ),
            &format!(
                "python -m pip install torchvision torchaudio -f https://download.pytorch.org/whl/{}",
                version.accelerator,
            ),
            "mkdir /app",
        ],
